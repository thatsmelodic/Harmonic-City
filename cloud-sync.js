...
const MAX_EMBEDDED_ASSET_BYTES=75*1024*1024;
...
async function uploadCurrentDevice(){if(!session){setStatus('Sign in first.');return false}if(syncing)return false;syncing=true;render();setStatus('Preparing layout and selected media…');try{const timestamp=new Date().toISOString();const settings=await payload();setStatus('Uploading layout and selected media to Harmonic Cloud…');const {data,error}=await supabase.from('portal_settings').upsert({owner_id:session.user.id,portal_key:PORTAL_KEY,settings,updated_at:timestamp},{onConflict:'owner_id,portal_key'}).select('updated_at').single();if(error)throw error;lastCloudUpdatedAt=parseTime(data?.updated_at||timestamp);const value=meta();value.cloudUpdatedAt=lastCloudUpdatedAt;value.localUpdatedAt=lastCloudUpdatedAt;writeJson(CLOUD_META_KEY,value);localDirty=false;setStatus(`Synced ${new Date().toLocaleTimeString()}. Layout and selected media are now in the cloud.`);return true}catch(error){console.error('Cloud sync failed',error);setStatus(`Sync failed: ${error.message}`);return false}finally{syncing=false;render()}}
