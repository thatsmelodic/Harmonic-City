...
function cleanAuthUrl(){history.replaceState({},document.title,location.pathname+location.search.replace(/[?&]code=[^&]*/,'').replace(/^\?&?/,'?').replace(/\?$/,''))}
...
async function consumeAuthCallback(){
  ...
  if(authError){cleanAuthUrl();throw new Error(decodeURIComponent(authError.replace(/\+/g,' ')))}
...
async function connectClient(){
  ...
  try{await consumeAuthCallback()}catch(error){console.error(error);setStatus(`Sign-in failed: ${error.message}`)}
...
function renderCloudState(){
  ...
  else{btn.textContent='☁ Connect Cloud';btn.classList.remove('cloud-connected');if(!cooldownRemaining())setStatus('Not connected. Request one sign-in email, then use the newest link only.')}
}
...
function bindUI(){
  ...
  sendLink.onclick=async()=>{
    ...
    sendLink.disabled=true;setStatus('Requesting one sign-in email…');
    const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:`${location.origin}${location.pathname}`}});
    startCooldown();
    setStatus(error?`Sign-in failed: ${error.message}`:'Email sent. Open the newest Harmonic City email; do not request another link.')
  };
