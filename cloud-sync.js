...
const NORMAL_COOLDOWN_SECONDS=90;
...
function cooldownRemaining(){const raw=Number(localStorage.getItem(MAGIC_LINK_SENT_KEY)||0);if(!raw)return 0;return Math.max(0,Math.ceil((raw-Date.now())/1000))}
function startCooldown(seconds=NORMAL_COOLDOWN_SECONDS){localStorage.setItem(MAGIC_LINK_SENT_KEY,String(Date.now()+seconds*1000));updateCooldownUI();clearInterval(cooldownTimer);cooldownTimer=setInterval(updateCooldownUI,1000)}
