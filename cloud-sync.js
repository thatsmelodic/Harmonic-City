...
let supabase=null,session=null,syncTimer=null,syncing=false,pendingSave=false,cooldownTimer=null,authListener=null,realtimeChannel=null,initialSyncStarted=false,lastRemoteUpdatedAt='',applyingRemote=false;
...
async function connectClient(){
  ...
  authListener=subscription.data.subscription;renderCloudState();if(session){startCrossDeviceSync();if(!initialSyncStarted)initialCloudSync()}return supabase;
}

function renderCloudState(){const btn=$('#cloudToggle');if(!btn)return;if(session){btn.textContent='☁ Cloud On';btn.classList.add('cloud-connected');setStatus(`Connected as ${session.user.email} • verifying sync…`)}else{btn.textContent='☁ Connect Cloud';btn.classList.remove('cloud-connected');if(!cooldownRemaining()&&!readAuthError())setStatus('Sign in to load and save Harmonic City across devices.')}}
...
async function uploadWorkspaceFile(workspace){const body=new Blob([JSON.stringify(workspace)],{type:'application/json'});const path=workspacePath();const result=await withTimeout(supabase.storage.from(STORAGE_BUCKET).upload(path,body,{contentType:'application/json',upsert:true,cacheControl:'no-cache'}),'Workspace upload');if(result.error)throw result.error;return {path,size:body.size}}
async function writeManifest(workspace,uploaded,syncComplete,mediaError=''){const manifest={version:WORKSPACE_VERSION,workspace_path:uploaded.path,workspace_size:uploaded.size,updated_at:workspace.updatedAt,device_id:deviceId(),sync_complete:syncComplete,media_error:mediaError||null};const {error}=await withTimeout(supabase.from('portal_settings').upsert({owner_id:session.user.id,portal_key:PORTAL_KEY,settings:manifest,updated_at:workspace.updatedAt},{onConflict:'owner_id,portal_key'}),'Workspace record');if(error)throw error;return manifest}

async function saveWorkspace(){
  if(!session){setStatus('Sign in to enable cloud saving.');return}
  if(syncing||applyingRemote){pendingSave=true;return}
  syncing=true;pendingSave=false;
  try{
    setStatus('Saving site settings…');
    const workspace=currentWorkspace();
    const uploaded=await uploadWorkspaceFile(workspace);
    await writeManifest(workspace,uploaded,true);
    lastRemoteUpdatedAt=workspace.updatedAt;
    ...
    try{
      ...
      await writeManifest(workspace,uploaded,true,mediaError.message).catch(console.error);
      setStatus(`Settings synced • media needs retry: ${mediaError.message}`);
    }
...
async function fetchRemoteRecord(){if(!session)return null;const {data,error}=await supabase.from('portal_settings').select('settings,updated_at').eq('owner_id',session.user.id).eq('portal_key',PORTAL_KEY).maybeSingle();if(error)throw error;return data||null}
async function applyRemoteWorkspace(record,{reload=true}={}){
  ...
  applyingRemote=true;
  try{
    setStatus('Updating from another device…');
    ...
    try{await downloadMissingMedia()}catch(mediaError){console.warn('Media restore incomplete',mediaError)}
    lastRemoteUpdatedAt=timestamp;
    setStatus('Updated from Harmonic Cloud');
    if(reload){sessionStorage.setItem('harmonic-city-cross-device-reload','1');location.reload()}
    return true;
...
async function uploadMissingMedia(){const items=await allLocalMedia();if(!items.length)return {uploaded:0,total:0};const {data:rows,error}=await supabase.from('portal_media').select('id').eq('owner_id',session.user.id).eq('portal_key',PORTAL_KEY);if(error)throw error;const existing=new Set((rows||[]).map(x=>x.id));const missing=items.filter(item=>!existing.has(item.id));let uploaded=0;for(let index=0;index<missing.length;index++){const item=missing[index];const sizeMB=item.blob?.size?` • ${(item.blob.size/1024/1024).toFixed(1)} MB`:'';setStatus(`Uploading media ${index+1} of ${missing.length}${sizeMB}…`);const path=`${session.user.id}/${PORTAL_KEY}/${item.kind}/${item.id}-${safeName(item.name)}`;const upload=await withTimeout(supabase.storage.from(STORAGE_BUCKET).upload(path,item.blob,{contentType:item.type,upsert:true,cacheControl:'31536000'}),`Media upload ${index+1}`);if(upload.error)throw upload.error;const {error:rowError}=await supabase.from('portal_media').upsert({id:item.id,owner_id:session.user.id,portal_key:PORTAL_KEY,kind:item.kind,name:item.name,storage_path:path,mime_type:item.type});if(rowError)throw rowError;uploaded++}return {uploaded,total:missing.length}}
async function downloadMissingMedia(){const local=await allLocalMedia();const existing=new Set(local.map(x=>x.id));const {data,error}=await supabase.from('portal_media').select('*').eq('owner_id',session.user.id).eq('portal_key',PORTAL_KEY).order('created_at',{ascending:false});if(error)throw error;const missing=(data||[]).filter(row=>!existing.has(row.id));for(let index=0;index<missing.length;index++){const row=missing[index];setStatus(`Restoring media ${index+1} of ${missing.length}…`);const {data:file,error:downloadError}=await withTimeout(supabase.storage.from(STORAGE_BUCKET).download(row.storage_path),`Media download ${index+1}`);if(downloadError)throw downloadError;await putLocalMedia({id:row.id,kind:row.kind,name:row.name,type:row.mime_type||file.type,blob:file,createdAt:new Date(row.created_at).getTime()})}}

function startCrossDeviceSync(){
  if(!supabase||!session||realtimeChannel)return;
  realtimeChannel=supabase.channel(`harmonic-city-${session.user.id}`)
    .on('postgres_changes',{event:'*',schema:'public',table:'portal_settings',filter:`owner_id=eq.${session.user.id}`},payload=>{
      const record=payload.new;if(!record||record.portal_key!==PORTAL_KEY)return;
      const manifest=record.settings||{};if(manifest.device_id===deviceId())return;
      applyRemoteWorkspace(record,{reload:true}).catch(error=>{console.error(error);setStatus(`Cross-device sync failed: ${error.message}`)});
    })
    .subscribe(status=>{if(status==='SUBSCRIBED')setStatus('Cloud connected • cross-device sync on')});
}
function stopCrossDeviceSync(){if(realtimeChannel&&supabase)supabase.removeChannel(realtimeChannel);realtimeChannel=null}

async function initialCloudSync(){if(initialSyncStarted||!session)return;initialSyncStarted=true;try{setStatus('Loading your cloud workspace…');const loaded=await loadWorkspace();if(loaded&&!sessionStorage.getItem('harmonic-city-cloud-reloaded')){sessionStorage.setItem('harmonic-city-cloud-reloaded','1');location.reload();return}await saveWorkspace()}catch(error){console.error(error);setStatus(`Cloud sync failed: ${error.message}`)}}
function scheduleSave(delay=900){if(!session||applyingRemote)return;clearTimeout(syncTimer);syncTimer=setTimeout(saveWorkspace,delay)}
