...
create policy "owners manage portal settings" on public.portal_settings for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);

drop policy if exists "owners manage portal media" on public.portal_media;
create policy "owners manage portal media" on public.portal_media for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);

drop policy if exists "owners manage portal versions" on public.portal_versions;
create policy "owners manage portal versions" on public.portal_versions for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
...
create policy "authenticated users upload harmonic city media" on storage.objects for insert to authenticated with check (bucket_id = 'harmonic-city-media' and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists "owners update harmonic city media" on storage.objects;
create policy "owners update harmonic city media" on storage.objects for update to authenticated using (bucket_id = 'harmonic-city-media' and owner_id = auth.uid()) with check (bucket_id = 'harmonic-city-media' and owner_id = auth.uid());

drop policy if exists "owners delete harmonic city media" on storage.objects;
create policy "owners delete harmonic city media" on storage.objects for delete to authenticated using (bucket_id = 'harmonic-city-media' and owner_id = auth.uid());
